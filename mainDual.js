...
const main = async() => {
    ...
    deriveTrend();
        ...
        const trailingStopControlUp = async(mode) => {                        
            if( mode !== "UP" ) { return }
            if(!kickStarted) { kickStarted = true }
            ...
            //Control selector..
            if( trendVector_BTCUP_ALMA200 < 1 && isDealClosed) {
                mode = "DOWN"
            }
        }
        ...
        const trailingStopControlDown = async(mode) => {            
            if( mode !== "DOWN" ) { return }
            if(!kickStarted) { kickStarted = true }
            console.log("DOWN active..")

            let trendVector_BTCDOWN_ALMA200 = 1 / trendVector_BTCUP_ALMA200
            let trendVector_BTCDOWN_ALMA20 = 1 / trendVector_BTCUP_ALMA20
            ...
            if( trendVector_BTCDOWN_ALMA200 > ALMA200_THRESHOLD && !isDealClosed) {      //UpTrend signifies an up-trend in ALMA200 Moving Average
                trailingStopPriceArrayDOWN.push(valueArray_BTCDOWN[1])                       //Creates and maintains price array. It gets RESET when trade is over..
                await buyAndSellOrder('BTCDOWN/USDT', 'buy')                             //Real monies are transferred into BTCUP..
            }
            ...
            //Control selector..                                        //Read explanation just below..
            if( trendVector_BTCDOWN_ALMA200 < 1 && isDealClosed) {
                mode = "UP"
            }
        }
